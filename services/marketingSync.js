...
const DEFAULT_PROVIDER = String(
  process.env.MARKETING_PROVIDER || "none",
).toLowerCase();
...
async function syncCustomerNewsletter(customer, wantNewsletter, opts = {}) {
  ...
  if (wantNewsletter && !email) {
    if (opts.mutate) {
      ...
      customer.marketingLastError = "Email required for subscription";
    }
    return {
      ok: false,
  ...
  try {
    ...
    if (opts.mutate) {
      ...
      if (["mailchimp", "brevo", "sendgrid"].includes(provider)) {
        customer.marketingProvider = provider;
      }
    ...
    const msg = e?.message || "Marketing sync failed";
    if (opts.mutate) {
      customer.marketingLastSyncedAt = new Date();
      customer.marketingLastError = msg;
    }

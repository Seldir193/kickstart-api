...
// function bookingRefScore(ref) {
//   let score = 0;

//   if (safeStr(ref?.bookingId)) score += 16;
//   if (safeStr(ref?.invoiceNumber)) score += 8;
//   if (safeStr(ref?.invoiceNo)) score += 6;
//   if (ref?.invoiceDate) score += 4;
//   if (safeStr(ref?.cancellationNo)) score += 2;
//   if (safeStr(ref?.stornoNo)) score += 2;
//   if (safeStr(ref?.childUid)) score += 2;
//   if (safeStr(ref?.offerTitle)) score += 1;
//   if (safeStr(ref?.offerType)) score += 1;
//   if (safeStr(ref?.venue)) score += 1;

//   return score;
// }

// function refsLookLikeSameBooking(ref, booking, childUid) {
//   const bookingId = safeStr(booking?._id);
//   if (!ref || !bookingId) return false;

//   if (sameObjectId(ref?.bookingId, bookingId)) return true;
//   if (sameObjectId(ref?._id, bookingId)) return true;

//   const refOfferId = safeStr(ref?.offerId);
//   const bookingOfferId = safeStr(booking?.offerId);
//   const refChildUid = safeStr(ref?.childUid);
//   const bookingChildUid = safeStr(childUid);

//   const refDate = dateKey(ref?.date);
//   const bookingDate = dateKey(booking?.date);

//   if (!refOfferId || !bookingOfferId) return false;
//   if (refOfferId !== bookingOfferId) return false;
//   if (!refChildUid || !bookingChildUid) return false;
//   if (refChildUid !== bookingChildUid) return false;
//   if (!refDate || !bookingDate) return false;

//   return refDate === bookingDate;
// }

// function findMatchingBookingRefs(customerDoc, booking, childUid) {
//   const list = Array.isArray(customerDoc?.bookings) ? customerDoc.bookings : [];
//   return list.filter((ref) => refsLookLikeSameBooking(ref, booking, childUid));
// }
...
// // function sameObjectId(a, b) {
// //   return safeStr(a) !== "" && safeStr(a) === safeStr(b);
// // }
...
// // function findBookingRef(customerDoc, booking, childUid) {
// //   const matches = findMatchingBookingRefs(customerDoc, booking, childUid);
// //   if (!matches.length) return null;

// //   return matches.sort((a, b) => bookingRefScore(b) - bookingRefScore(a))[0];
// // }
...
//   const matches = findMatchingBookingRefs(customerDoc, booking, childUid);
//   const ref =
//     matches.sort((a, b) => bookingRefScore(b) - bookingRefScore(a))[0] || null;
...
//     childUid: safeStr(childUid),
//     childFirstName:
//       safeStr(child?.firstName) || safeStr(booking?.childFirstName),
//     childLastName: safeStr(child?.lastName) || safeStr(booking?.childLastName),
...
//     stornoAmount:
//       typeof ref?.stornoAmount === "number" ? ref.stornoAmount : null,
//     invoiceRefs: Array.isArray(ref?.invoiceRefs) ? ref.invoiceRefs : [],
//   };
...
//   customerDoc.bookings = customerDoc.bookings.filter((item) => {
...
//   const gender = safeStr(meta.gender) || safeStr(invChild.gender) || "";
//   const club = safeStr(meta.club) || safeStr(invChild.club) || "";
...
// function findChildUid(customerDoc, child) {
//   const list = ensureChildrenArray(customerDoc);
//   const key = childKey(child);
//   const found = list.find((c) => childKey(c) === key);
//   if (found) return ensureUid(found);
//   return "";
// }
...
//   if (hasUid) {
//     const hit = list.find((c) => safeStr(c?.uid) === child.uid);
//     if (hit) {
//       ensureUid(hit);
//       if (!safeStr(customerDoc.child?.uid)) customerDoc.child = hit;
//       if (hasName) {
//         if (!safeStr(hit.firstName)) hit.firstName = child.firstName;
//         if (!safeStr(hit.lastName)) hit.lastName = child.lastName;
//       }
//       return safeStr(hit.uid);
//     }

//     child.uid = ensureUid(child);
//     list.push(child);
//     if (!safeStr(customerDoc.child?.uid)) customerDoc.child = child;
//     return safeStr(child.uid);
//   }

//   const existingUid = findChildUid(customerDoc, child);
...
//   if (customerDoc.child && childKey(customerDoc.child) === childKey(child)) {
//     ensureUid(customerDoc.child);
//   }
...
// function findBookingRef(customerDoc, bookingId) {
//   const id = String(bookingId || "");
//   const a = customerDoc.bookings?.find((b) => String(b?.bookingId) === id);
//   if (a) return a;
//   return customerDoc.bookings?.find((b) => String(b?._id) === id) || null;
// }
...
//   if (!ref) return void customerDoc.bookings.push(next);
//   Object.assign(ref, next);
// }
...
//     const lower = String(emailLower).trim().toLowerCase();
//     if (lower && existing.emailLower !== lower) {
//       existing.emailLower = lower;
//       changed = true;
//     }

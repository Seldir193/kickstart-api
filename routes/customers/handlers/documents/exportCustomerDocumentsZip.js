...
function buildBillingInvoiceDocs(customer, billingDocs) {
  ...
  const bookingById = new Map(
    refs.map((b) => [safeText(b?.bookingId || b?._id), b]),
  );
  const out = [];
...
async function exportCustomerDocumentsZip(
  ...
  try {
    ...
    async function appendInvoicePdf(d) {
      const filePath = safeText(d?.filePath);
      if (!filePath) return false;

      try {
        const buf = await fs.readFile(filePath);
        const base = safeText(d.fileName || d.invoiceNo || d.title || d.id);
        const name = /\.pdf$/i.test(base) ? base : `${base}.pdf`;
        archive.append(buf, { name: safe(name) });
        return true;
      } catch (err) {
        const msg = `Invoice PDF read failed for ${d.id || "unknown"}: ${err?.message || "read error"}`;
        archive.append(Buffer.from(msg, "utf8"), {
          name: `error-${d.bookingId || "unknown"}-invoice.txt`,
        });
        return true;
      }
    }
    ...
    async function processDoc(d) {
      if (d.type === "invoice") {
        const handled = await appendInvoicePdf(d);
        if (handled) return;
      }
      if (d.type === "dunning") {

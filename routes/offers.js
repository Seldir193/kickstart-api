...
function validateOfferBody(b) {
  ...
  if (
    b.ageFrom != null &&
    b.ageTo != null &&
    Number(b.ageFrom) > Number(b.ageTo)
  ) {
...
router.get("/", async (req, res) => {
  try {
    ...
    if (String(includeArchived) !== "1") {
      base.archivedAt = null; // match null OR missing
    }
...
router.post("/", async (req, res) => {
  try {
    ...
    const title = [offerDisplayName(b), locationStr]
      .filter(Boolean)
      .join(" • ");
    ...
    const doc = await Offer.create({
      ...
      ageFrom:
        b.ageFrom === "" || b.ageFrom == null ? undefined : Number(b.ageFrom),
      ...
      // 🔹 NEU: Ferien-Felder
      holidayWeekLabel: b.holidayWeekLabel
        ? String(b.holidayWeekLabel).trim()
        : "",
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const doc = await Offer.findOneAndUpdate({ _id: id, owner }, updateOps, {
      new: true,
    }).lean();
...
router.patch("/:id", async (req, res) => {
  try {
    ...
    if ("ageFrom" in update)
      ops.$set.ageFrom =
        update.ageFrom === "" || update.ageFrom == null
          ? undefined
          : Number(update.ageFrom);
    if ("ageTo" in update)
      ops.$set.ageTo =
        update.ageTo === "" || update.ageTo == null
          ? undefined
          : Number(update.ageTo);
    if ("info" in update)
      ops.$set.info = update.info ? String(update.info) : "";
    if ("onlineActive" in update) ops.$set.onlineActive = !!update.onlineActive;

    if ("coachName" in update)
      ops.$set.coachName = String(update.coachName || "").trim();
    if ("coachEmail" in update)
      ops.$set.coachEmail = String(update.coachEmail || "").trim();
    if ("coachImage" in update)
      ops.$set.coachImage = String(update.coachImage || "").trim();

    if ("category" in update)
      ops.$set.category = String(update.category || "").trim();
    if ("sub_type" in update)
      ops.$set.sub_type = String(update.sub_type || "").trim();
    if ("legacy_type" in update)
      ops.$set.legacy_type = String(update.legacy_type || "").trim();
    ...
    const doc = await Offer.findOneAndUpdate({ _id: id, owner }, ops, {
      new: true,
      runValidators: true,
    }).lean();
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    const doc = await Offer.findOneAndUpdate(
      { _id: id, owner },
      { $set: { onlineActive: false, archivedAt: now } },
      { new: true },
    ).lean();
    if (!doc) return res.status(404).json({ error: "Offer not found" });
    return res.json({ ok: true, id });
